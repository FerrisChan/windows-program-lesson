...
void HashCache::put(string key, string value) {
	std::lock(get_mutex, put_mutex);
	std::lock_guard<std::mutex>  lock1(get_mutex, std::adopt_lock);
	std::lock_guard<std::mutex>  lock2(put_mutex, std::adopt_lock);
    Node* node = NULL;
    node = search(key);
    if (node) {  // 存在当前key
...
int HashCache::load_file_to_page(const int &file_num, Page* &page, unordered_map<string, Node* > &hash_map) {
    ifstream input;
    string file_name = to_string(file_num) + ".txt"; // 用文件编号构造文件名
